...
M_reactants = 0.029  # Molar mass of reactants (kg/mol, approx. for air-like mixture)
...
blockage_ratio = 0.5  # 50% blockage at obstacles

# Spatial grid
x = np.linspace(0, L, N)

# Define cross-sectional area (A) with obstacles
A = np.ones(N)  # Default area is 1.0
obstacle_positions = np.arange(1, 10, 1)  # Obstacles every 1 meter
obstacle_width = 0.1  # Obstacle width (m)

for pos in obstacle_positions:
    # Create a mask for positions within obstacle width
    mask = (x >= pos - obstacle_width / 2) & (x <= pos + obstacle_width / 2)
    A[mask] = 1 - blockage_ratio  # Reduce area at obstacles
...
rho_init = 1.2  # Initial density (kg/m^3)
U[0] = rho_init * A  # Mass: rho*A
U[1] = 0.0  # Momentum: rho*u*A (initially at rest)
U[2] = rho_init * A * (cv * 300 + q0)  # Total energy: rho*E*A, T=300K + chemical energy
U[3] = 0.0  # Reaction progress: rho*c*A (0 = unburnt)
U[3, :N//10] = rho_init * A[:N//10]  # Ignite left 10% of the domain (c=1)
